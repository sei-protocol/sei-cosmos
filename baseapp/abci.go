...
func (app *BaseApp) ProcessProposal(ctx context.Context, req *abci.RequestProcessProposal) (*abci.ResponseProcessProposal, error) {
	...
	if maxGas := app.getMaximumBlockGas(app.processProposalState.ctx); maxGas > 0 {
		gasMeter = sdk.NewGasMeterWithLogger(maxGas, app.processProposalState.ctx.Logger(), "ProcessProposal-Block")
	} else {
		gasMeter = sdk.NewInfiniteGasMeterWithLogger(app.processProposalState.ctx.Logger(), "ProcessProposal-Block")
	}
...
func (app *BaseApp) FinalizeBlock(ctx context.Context, req *abci.RequestFinalizeBlock) (*abci.ResponseFinalizeBlock, error) {
	...
	if maxGas := app.getMaximumBlockGas(app.deliverState.ctx); maxGas > 0 {
		gasMeter = sdk.NewGasMeterWithLogger(maxGas, app.deliverState.ctx.Logger(), "FinalizeBlock-Block")
	} else {
		gasMeter = sdk.NewInfiniteGasMeterWithLogger(app.deliverState.ctx.Logger(), "FinalizeBlock-Block")
	}
