...
func TestWithRouter(t *testing.T) {
	...
	for blockN := 0; blockN < nBlocks; blockN++ {
		...
		app.deliverState.ctx = app.deliverState.ctx.WithBlockGasMeter(sdk.NewInfiniteGasMeterWithLogger(&NoOpLogger{}, "test"))
...
func TestSimulateTx(t *testing.T) {
	...
	anteOpt := func(bapp *BaseApp) {
		bapp.SetAnteHandler(func(ctx sdk.Context, tx sdk.Tx, simulate bool) (newCtx sdk.Context, err error) {
			newCtx = ctx.WithGasMeter(sdk.NewGasMeterWithLogger(gasConsumed, &NoOpLogger{}, "test"))
			return
		})
	}
...
func TestTxGasLimits(t *testing.T) {
	...
	anteOpt := func(bapp *BaseApp) {
		bapp.SetAnteHandler(func(ctx sdk.Context, tx sdk.Tx, simulate bool) (newCtx sdk.Context, err error) {
			newCtx = ctx.WithGasMeter(sdk.NewGasMeterWithLogger(gasGranted, &NoOpLogger{}, "test"))
...
func TestMaxBlockGasLimits(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		header := tmproto.Header{Height: app.LastBlockHeight() + 1}
		app.setDeliverState(header)
		app.deliverState.ctx = app.deliverState.ctx.WithBlockGasMeter(sdk.NewGasMeterWithLogger(app.getMaximumBlockGas(app.deliverState.ctx), &NoOpLogger{}, "test"))
		app.BeginBlock(app.deliverState.ctx, abci.RequestBeginBlock{Header: header})
...
func TestGasConsumptionBadTx(t *testing.T) {
	...
	anteOpt := func(bapp *BaseApp) {
		bapp.SetAnteHandler(func(ctx sdk.Context, tx sdk.Tx, simulate bool) (newCtx sdk.Context, err error) {
			newCtx = ctx.WithGasMeter(sdk.NewGasMeterWithLogger(gasWanted, &NoOpLogger{}, "test"))
...
func TestCheckTx(t *testing.T) {
	...
	header := tmproto.Header{Height: 1}
	app.setDeliverState(header)
	app.checkState.ctx = app.checkState.ctx.WithBlockGasMeter(sdk.NewInfiniteGasMeterWithLogger(&NoOpLogger{}, "test")).WithHeaderHash([]byte("hash"))
	app.deliverState.ctx = app.deliverState.ctx.WithBlockGasMeter(sdk.NewInfiniteGasMeterWithLogger(&NoOpLogger{}, "test"))
