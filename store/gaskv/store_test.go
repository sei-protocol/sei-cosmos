...
func TestGasKVStoreBasic(t *testing.T) {
	...
	meter := types.NewGasMeterWithLogger(10000, &types.NoOpLogger{}, "test")
...
func TestGasKVStoreIterator(t *testing.T) {
	mem := dbadapter.Store{DB: dbm.NewMemDB()}
	meter := types.NewGasMeterWithLogger(100000, &types.NoOpLogger{}, "test")
	st := gaskv.NewStore(mem, meter, types.KVGasConfig())
...
func TestGasKVStoreOutOfGasSet(t *testing.T) {
	mem := dbadapter.Store{DB: dbm.NewMemDB()}
	meter := types.NewGasMeterWithLogger(0, &types.NoOpLogger{}, "test")
	st := gaskv.NewStore(mem, meter, types.KVGasConfig())
...
func TestGasKVStoreOutOfGasIterator(t *testing.T) {
	mem := dbadapter.Store{DB: dbm.NewMemDB()}
	meter := types.NewGasMeterWithLogger(20000, &types.NoOpLogger{}, "test")
	st := gaskv.NewStore(mem, meter, types.KVGasConfig())
	st.Set(keyFmt(1), valFmt(1))
	iterator := st.Iterator(nil, nil)
	iterator.Next()
