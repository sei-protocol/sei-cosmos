...
func TestInfiniteGasMeter(t *testing.T) {
	t.Parallel()
	meter := NewInfiniteGasMeterWithLogger(&NoOpLogger{}, "test")
...
func TestGasMeter(t *testing.T) {
	...
	for tcnum, tc := range cases {
		meter := NewGasMeterWithLogger(tc.limit, &NoOpLogger{}, "test")
		used := uint64(0)
		...
		meter2 := NewGasMeterWithLogger(math.MaxUint64, &NoOpLogger{}, "test")
