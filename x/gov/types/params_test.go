...
func TestTallyParamsGetThreshold(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			tallyParams:   types.NewTallyParams(types.DefaultQuorum, types.DefaultExpeditedQuorum, types.DefaultThreshold, sdk.NewDecWithPrec(777, 3), types.DefaultVetoThreshold),
			expectedValue: sdk.NewDecWithPrec(777, 3),
			isExpedited:   true,
		},
		{
			name:          "default not expedited",
			tallyParams:   types.NewTallyParams(types.DefaultQuorum, types.DefaultExpeditedQuorum, sdk.NewDecWithPrec(6, 1), types.DefaultExpeditedThreshold, types.DefaultVetoThreshold),
